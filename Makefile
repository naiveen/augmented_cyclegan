...
# Update the Makefile from GitHub, by cloning repository through ssh.
# Once the repo has been cloned, a subshell is launched in the background which
# will sleep a little bit to give a change to `make` to exit. Then, the new file
# is moved in place. This should prevent problem when overwriting the Makefile
# while `make` is running. This can also be racy though...
.PHONY: update
update:
	@read -p "Self-updating the makefile. Continue? [y/N] " answer && \
	    if [[ "$${answer}" == "Y" || "$${answer}" == "y" || "$${answer}" == "yes" ]]; then \
	        rm -fr .makefile-docker-template.git; \
	        git clone --branch=$(UPDATE_BRANCH) --no-checkout --depth 1 $(UPDATE_GIT_REPOSITORY) .makefile-docker-template.git && \
	        pushd .makefile-docker-template.git && \
	        git show HEAD:Makefile > $(PWD)/.new_Makefile && \
	        popd && \
	        rm -fr .makefile-docker-template.git && \
	        ( ( \
	            sleep 0.1 && \
...
# #############################################################################
# Print debug command (allows to print to `make print-VARIABLE` to output 
